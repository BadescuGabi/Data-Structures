...
int mediana_2_vectori(int a[], int b[], int size) {
    ...
    if (size == 2) {
        if(a[0]<b[0])
            return min(max(a[0],a[1]),min(b[0],b[1]));
            return min(max(b[0],b[1]),min(a[1],a[0]));
    } else {
        int med_a = a[size / 2];
        int med_b = b[size / 2];
        if (med_a < med_b) {
            if (size % 2 ==
                0) {//ia a2a jumatate a primului vector si prima a celui de al 2 lea iar la al 2 lea if invers
